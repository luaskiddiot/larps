document.addEventListener("DOMContentLoaded", () => {
    ...
    function live() {
        ...
        // Force Sidebar and Greeting to always be visible
        ['greet','bar'].forEach(id => {
            const el = document.getElementById(id);
            if(el) {
                el.style.opacity = '1';
        ...
        // Handle Weather visibility (only depends on wEn now)
        const wthr = document.getElementById('wthr');
        if(wthr) {
            if(!cfg.wEn) {
                wthr.style.opacity = '0';
                // Optional: set pointerEvents to none if hidden, though not strictly required if opacity is 0
            } else {
                wthr.style.opacity = '1';
                wthr.style.pointerEvents = 'auto';
            }
        }
    }
